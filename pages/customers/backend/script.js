document.addEventListener("DOMContentLoaded", function () {
    ...
    let markers = [];
    let productLocations = []; // Stores real-time order locations
    ...
    function loadGoogleMaps() {
        ...
        script.src = `https://maps.googleapis.com/maps/api/js?key=FCMG122HIxrqLa3oR5RXr9rsMHSs65GzUfIozYD_5yM&callback=initMap`;
        script.defer = true;
    ...
    function geocodeAddress(orderID, address, quantity) {
        ...
        geocoder.geocode({ address: address }, function (results, status) {
            if (status === "OK") {
                ...
                productLocations.push(newMarker);
                addMarker(newMarker);
            } else {
    ...
    document.getElementById("viewMapBtn").addEventListener("click", function () {
        if (productLocations.length === 0) {
            alert("No locations to show on the map!");
            return;
        }
    
        // Convert order locations to JSON and encode it for the URL
        let locationData = encodeURIComponent(JSON.stringify(productLocations));
        window.open(`fullmap.html?data=${locationData}`, "_blank");
    });
    // Open Full-Screen Map with Plotted Markers
    document.getElementById("mapContainer").addEventListener("click", function () {
        let baseUrl = "https://www.google.com/maps/dir/";
        let markersQuery = productLocations.map(loc => `${loc.lat},${loc.lng}`).join("/");
        let fullMapUrl = `${baseUrl}${markersQuery}`;
